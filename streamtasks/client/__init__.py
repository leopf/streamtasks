...
from streamtasks.client.discovery import DISCOVERY_CONSTANTS, GenerateTopicsRequestBody, GenerateTopicsResponseBody, ResolveAddressRequestBody, ResolveAddressResonseBody
...
from streamtasks.services.constants import NetworkAddresses, NetworkPorts
...
class Client:
  ...
  async def resolve_address_name(self, name: str) -> Optional[int]:
    ...
    raw_res = await self.fetch(NetworkAddresses.ID_DISCOVERY, DISCOVERY_CONSTANTS.RESOLVE_ADDRESS, ResolveAddressRequestBody(address_name=name).model_dump())
  ...
  async def request_topic_ids(self, count: int, apply: bool = False) -> list[int]:
    raw_res = await self.fetch(NetworkAddresses.ID_DISCOVERY, DISCOVERY_CONSTANTS.REQUEST_TOPICS, GenerateTopicsRequestBody(count=count).model_dump())
