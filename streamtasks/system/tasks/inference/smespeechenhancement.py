...
class SMESpeechEnhancementTask(SyncTask):
  ...
  def run_sync(self):
    model = SpectralMaskEnhancement.from_hparams(self.config.source, savedir=get_model_data_dir(self.config.source), run_opts={ "device":self.config.device })
